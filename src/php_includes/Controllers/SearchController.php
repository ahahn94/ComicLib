 ...
 * Implements the controller for the volumes search view.
 */
class SearchController implements Controller
{
    ...
    private $searchText;        // The search text to for reusing in pagination.
    ...
    public function __construct($path, $getParameters)
    {
        ...
        if (isset($searchText)) {
            ...
            $this->searchText = urlencode($searchText); // Save search text for reuse and urlencode to enable adding to URL.

            // Prepare data for view.
            $volumesRepo = new VolumeReadStatus();
            $volumes = $volumesRepo->search($userID, $searchText);
    ...
    {
        // Send view VolumesView. Empty $volumes will be handled there.
        include $_SERVER["DOCUMENT_ROOT"] . "/php_includes/Views/SearchView.php";
    }
}